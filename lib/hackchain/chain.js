...
Chain.prototype.getTXSpentBy = function getTXSpentBy(hash, callback) {
  this.db.get('tx/' + hash.toString('hex') + '/spentby', (err, txHash) => {
    if (err)
      return callback(err);

    this.getTX(txHash, callback);
  });
};
...
Chain.prototype.storeBlock = function storeBlock(block, callback) {
  ...
  const storeTXSpentBy = (tx, callback) => {
    ...
    async.forEach(tx.inputs, (input, callback) => {
      this.db.put('tx/' + input.hash.toString('hex') + '/spentby', hash,
                  callback);
    }, callback);
  };
