...
Chain.prototype.getRawBlock = function getRawBlock(hash, callback) {
  if (this.db === null)
    return callback(new Error('Chain: DB is closed'));
  this.db.get(this._blockKey(hash.toString('hex')), callback);
};
...
Chain.prototype.getRawTX = function getRawTX(hash, callback) {
  if (this.db === null)
    return callback(new Error('Chain: DB is closed'));

  this.db.get(this._txKey(hash.toString('hex')), callback);
};
