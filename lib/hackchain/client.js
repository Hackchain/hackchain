...
Client.prototype.compileScript = function compileScript(script) {
  const asm = new Assembler();

  for (let i = 0; i < script.length; i++) {
    let line = script[i].split(/\s+/g);
  ...
  return new TX.Script(code);
};

Client.prototype.parseTX = function parseTX(txt, callback) {
  ...
  for (let i = 0; i < data.inputs.length; i++) {
    ...
    const script = this.compileScript(input.script);
  ...
  for (let i = 0; i < data.outputs.length; i++) {
    ...
    const script = this.compileScript(output.script);
