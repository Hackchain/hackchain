...
RouteHandler.prototype.getBlock = function getBlock(hash) {
  this.server.chain.getBlock(new Buffer(hash, 'hex'), (err, block) => {
    if (err)
...
RouteHandler.prototype.getTX = function getTX(hash) {
  this.server.chain.getTX(new Buffer(hash, 'hex'), (err, tx) => {
    if (err)
