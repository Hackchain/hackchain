...
RouteHandler.prototype.run = function run() {
  ...
  if (req.method === 'GET') {
    ...
    let match = req.url.match(/^\/v1\/(tx|block)\/((?:[a-f0-9]{2})+)$/);
    if (match !== null) {
      if (match[1] === 'block')
    ...
    match = req.url.match(/^\/v1\/tx\/((?:[a-f0-9]{2})+)\/(block|spentby)$/);
    if (match !== null) {
      if (match[2] === 'block')
        return this.getTXBlock(match[1]);
      else
        return this.getTXSpentBy(match[1]);
    }
  } else if (req.method === 'POST') {
    let match = req.url.match(/^\/v1\/tx\/((?:[a-f0-9]{2})+)$/);
    if (match !== null)
...
RouteHandler.prototype.getTXSpentBy = function getTXSpentBy(hash) {
  this.server.chain.getTXSpentBy(Buffer.from(hash, 'hex'), (err, hash) => {
    if (err)
      return this.respond(404, { error: 'TX not found' });
